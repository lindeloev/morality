...
TO DO:
 * Check "timeLimitPassed" behavior and loop=0 vs. 1
 * Column order messes up for dots-only and questionnaire?
...
glucoseTime = 2 * 60  # how much CS + PGG-self time before glucose measure is taken.
...
DIALOGUE.update({'timeLimit': 3348})  # from 1-7 data. Corresponds to 55.8 minutes.
...
if DIALOGUE['level'] == '1-7':
    ...
    repetitions = {'experiment': 9, 'practiceExp': 1, 'practiceCS': 1}  # number of repetitions of each trial

# Estimated times:
"""
Mean trial duration before glucose version: 32.8 seconds!
12 trials per repetition
xx secs for 1-7. AVG 32.8. 3600  secs / 32.8 / 12 trials = 9.14 repetitions.
48 secs for 1-3. AVG 16. 3600 secs / 16 secs / 12 trials = 18.8 repetitions.
95 secs for 3-5 AVG 31.7. 3600 secs / 31.7 secs / 12 trials = 9.5 repetitions.
148 secs for 5-7 AVG 49.3. 3600 secs / 49.3 secs / 12 trials = 6.0 repetitions.
"""
if DIALOGUE['level'] == '1-3':
    spans = {'experiment': [1, 2, 3], 'practiceExp': [2, 1, 3], 'practiceCS': [2, 1, 3, 1, 3, 2]}  # span lengths
    repetitions = {'experiment': 19}  # number of repetitions of each trial
if DIALOGUE['level'] == '3-5':
    spans = {'experiment': [3, 4, 5], 'practiceExp': [4, 3, 5], 'practiceCS': [3, 4, 5, 4, 3, 5]}  # span lengths
    repetitions = {'experiment': 9}  # number of repetitions of each trial
if DIALOGUE['level'] == '5-7':
    spans = {'experiment': [5, 6, 7], 'practiceExp': [6, 5, 7], 'practiceCS': [5, 6, 7, 5, 7, 6]}  # span lengths
    repetitions = {'experiment': 6}  # number of repetitions of each trial

spans.update({'practiceDots': [1], 'controlDotsCheat': [1], 'controlDotsAccuracy': [1]})
repetitions.update({'practiceCS': 1, 'practiceExp': 1, 'practiceDots': 6, 'controlDotsCheat': 14, 'controlDotsAccuracy': 14})
...
def makeTrials(condition):
    ...
    if condition[0:8] == 'practice':
        dots_conditions = [None]  # not factorial, just pick randomly
    elif condition == 'controlDotsCheat':
        dots_conditions = ['cheat']
    elif condition == 'controlDotsAccuracy':
        dots_conditions = ['accuracy']
    else:
        dots_conditions = ['cheat', 'cheat', 'cheat', 'accuracy']
    ...
    for rep in range(repetitions[condition]):  # number of repetitions is specified by condition
        for span in spans[condition]:  # span lengths depend on condition
            for dots_incentive in dots_conditions:
                # For practice, pick random dot conditions without doing factorial
                dots_N = random.choice(dots_Ns)
...
moneyCumulative = 0
def runFeedback(trial, trial_time, trial_list_length):
    ...
    if trial['condition'] == 'experiment':
        text_feedback += '\n\n%i%% finished.' %min(100*trial_time / DIALOGUE['timeLimit'], 100*trial['no']/trial_list_length)  # experiment goes to 85%, then control and questionnaire
...
def runBlock(condition):    
    ...
    # Loop until timelimit
    clock_block.reset()
    
    loops = range(1000) if condition == 'experiment' else [0]
    for loop in loops:
        trialList = makeTrials(condition)
        for trialNumber, trial in enumerate(trialList):
            trial['no'] += loop * len(trialList)  # make trial number continuous on loops. Would otherwise restart at 1.
            clock.reset()
            ...
            # Feedback and break
            trial = runFeedback(trial, clock_block.getTime(), len(trialList))
            
            # Save trial
            saveTrial(trial)
            
        
            # Return if time's up (also if the block is not finished).
            if condition == 'experiment' and DIALOGUE['timeLimit'] and clock_block.getTime() > DIALOGUE['timeLimit']:
