---
title: "Morality in the time of cognitive famine - analysis"
...
* Fix colors in figure 4+5
* Dots depletion plot missing early section?
* Dots - having stimType as random makes no sense since it has only two values?
-->
...
# Experiment 3 and 4: Dots task and cheating
```{r}
...
# LRT_binom(D,
#           dotsCorrect ~ dotsIncentive + (1|id) + (1|span) + (1|dotsN),
#           dotsCorrect ~             1 + (1|id) + (1|span) + (1|dotsN))

# Notice that the reference level becomes quite a specific combination of the fixed effects,
# But we are only interested in the effects, not the intercept, so that's OK.
LRT_binom(D,
...
```{r}
# LRT_binom(D,
#           dotsCorrect ~ dotsIncentive*dotsReceiver + (1|id) + (1|span) + (1|dotsN),
#           dotsCorrect ~ dotsIncentive+dotsReceiver + (1|id) + (1|span) + (1|dotsN))
...
```{r}
# LRT_binom(D,
#           dotsCorrect ~ dotsIncentive*span + (1|id) + (1|dotsN) + (1|dotsReceiver),
#           dotsCorrect ~ dotsIncentive+span + (1|id) + (1|dotsN) + (1|dotsReceiver))
...
```{r}
# LRT_binom(D,
#           dotsCorrect ~ dotsIncentive*span*dotsN + (1|id),
#           dotsCorrect ~ dotsIncentive*span*dotsN + (1|id) - dotsIncentive:span:dotsN)
...
#           dotsCorrect ~ time_hours + (1|id) + (1|dotsN) + (1|dotsReceiver),
#           dotsCorrect ~ (1|id) + (1|dotsN) + (1|dotsReceiver))
# 
...
```{r}
# LRT_binom(subset(D_time, level != '1-7'),
#           dotsCorrect ~ time_hours*dotsIncentive*level + dotsN + dotsReceiver + (span|id),
#           dotsCorrect ~ time_hours*dotsIncentive*level + dotsN + dotsReceiver + (span|id) - time_hours:dotsIncentive:level)
...
```{r dots questionnaire level, cache=T, message=F, warning=F}
...
result_level_dots = brms::brm(perceived_load ~ level, D.analyse.dots)  # Quick: just run with the defaults
...
```{r dots questionnaire invest, cache=T, message=F, warning=F}
# Plot it
...
```{r}
# Inferentially
result_cheat_dots = brms::brm(perceived_load ~ dotsCorrect, D.analyse.dots)  # Simple linear regression
